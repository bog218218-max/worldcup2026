...
- `/start` - показ панели кнопок для заранее добавленного участника
- `Матчи` - список открытых матчей и inline-кнопки для прогноза
- `/predict <matchId> <счёт>` - прогноз по ID матча, пример: `/predict cmatchid 2:1`
- `Мои прогнозы` - свои прогнозы и статус начисления очков
- `Таблица` - таблица лидеров
- `Правила` - правила начисления и дедлайны
- `Изменить имя` - смена display name на сайте
- `Помощь` - краткая инструкция

Участники не создаются автоматически. Если `telegramId` не добавлен админом, бот отвечает: `Ты не добавлен в список участников турнира. Напиши админу.`

Основной пользовательский сценарий не требует slash-команд: участник нажимает `Матчи`, выбирает матч кнопкой под списком и отправляет счёт в формате `2:1`.

Админские команды:

- `/admin` - список админских команд
- `/add_participant <telegramId>; <displayName>` - добавить или восстановить участника, сразу `isPaid=true`
- `/import_participants` - добавить или восстановить участников пачкой
- `/remove_participant <telegramId>` - soft-deactivation через `isPaid=false`, без удаления User и прогнозов
- `/missing <matchId>` - список активных участников без прогноза на матч
...
- `/set_result <matchId> <счёт>` - внести результат, пересчитать очки и получить мини-отчёт
- `/recalc CONFIRM` - пересчитать все завершённые матчи
- `/participants` - список участников
- `/prize` - призовой фонд

Админские команды доступны по `ADMIN_TELEGRAM_IDS`, даже если админа нет в `User`. Прогнозировать админ может только если он отдельно добавлен как участник с `isPaid=true`.

Bulk import участников:
...
Если хотя бы одна строка неверная, бот не импортирует никого и покажет номер строки с ошибкой.
...
4. Не запускать seed, `db:clear-demo` или import-скрипты после появления реальных матчей, участников или прогнозов.
5. Перезапустить сервисы: `docker compose -p world-cup-predictions -f docker-compose.prod.yml --env-file .env up -d`.
6. Добавить участников через `/add_participant <telegramId>; <displayName>`.
7. Добавить реальные матчи через `/add_match Команда1; Команда2; YYYY-MM-DD HH:mm; Stage`, если они ещё не импортированы.
8. Проверить кнопку `Матчи` в боте.
9. Дать друзьям ссылку на бота.
10. Проверить, что неизвестный пользователь на `/start` не создаётся и получает отказ.
11. Проверить `/participants`.
